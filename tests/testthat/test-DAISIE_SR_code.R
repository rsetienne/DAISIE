test_that("Torsten's Lake Biwa analyses work", {
  Biwa_datalist = NULL
  rm(Biwa_datalist)
  ...
  loglik <- DAISIE_SR_loglik_CS(pars1 = pars1,
                               pars2 = pars2,
                               datalist = Biwa_datalist) # -232.0618
  testthat::expect_equal(loglik, -232.0618, tol = 1E-3)
  
  # Simulate fish diversity over 4 Ma
  M <- 312
  IslandAge <- 4
  sims <- DAISIE_SR_sim(time = 4, M = M - 17, sample_freq = 25, pars = Pars,
                        replicates = 10, ddep = 11 )
  # Enter a test here, with expect.equal
  ...
  # No shift in anagenetic rate older any known non-endemic
  pars1mat[7,5] <- 0.01; pars1mat[7,10] <- 0.1;
  pars1mat[8,5] <- 0.01; pars1mat[8,10] <- 0.1; pars1mat[8,11] <- 2.9
  for(i in 1:8)
  {
     loglik <- DAISIE_SR_loglik_CS(pars1 = pars1mat[1,],
                      pars2 = pars2,
                      datalist = Macaronesia_datalist[[2]])
     testthat::expect.equal(loglik,-Inf)
  }
})

...
public:
  ...
  void rhs(const double* x, double* /* inout */ rdx);
...
template <>
void cpp_daisie_iw<3>::rhs(const double* rx, double* rdx)
{
...
template <>
void cpp_daisie_iw<4>::rhs(const double* rx, double* rdx)
{
  const auto dim_c = c_[0].dimensions();
  tmap dx(rdx, dim_c);
  std::array<std::pair<int, int>, rank> xxpad = {
...
struct daisie_iw_wrapper
{
  ...
  // odeint interface
  void operator()(const std::vector<double>& x, std::vector<double>& dxdt, double) const
  {
    (iw3) ? iw3->rhs(x.data(), dxdt.data())
          : iw4->rhs(x.data(), dxdt.data());
  }
};

#' @keywords internal
TRAISIE_update_state_CS  <- function(timeval,
                                                  total_time,
                                                  possible_event,
                                                  maxspecID,
                                                  island_spec,
  ...
  for (i in 0:(n - 1)) {
    ...
    if (possible_event == (4 * i + 2)) {
      ...
      if (typeofspecies == "C") {
        ...
        if (length(sisters) >= 3) {
          ...
          if (mostrecentspl == "A") {
            tochange = possiblesister[
              which(island_spec[possiblesister, 6] ==
    ...
    if (possible_event == (4 * i + 3)) {
      immi_specs = intersect(
        which(island_spec[, 4] == "I"),
        which(island_spec[, 8] == as.character(trait_state))
      )
      ...
      # Step 3: check if the immigrant species sampled (anagenesis) is unique
      # of if it is a recolonist
    ...
    if (possible_event == (4 * i + 4)) {
      ...
      if (island_spec[tosplit, 4] == "C") {
        ...
        island_spec[tosplit, 9] = paste(as.character(oldsplit),
                                        as.character(split_time), sep = " ")

        island_spec = rbind(island_spec,
                            c(maxspecID + 2,
                              island_spec[tosplit, 2],

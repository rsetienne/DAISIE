...
DAISIE_loglik_CS_M1 <- DAISIE_loglik <- function(pars1,
  ...
  if (is.na(island_ontogeny)) {
    # This calls the old code that doesn't expect
    # ontogeny
    lac <- pars1[1]
    mu <- pars1[2]
    K <- pars1[3]
  ...
  if (lac == Inf & missnumspec == 0 & length(pars1) == 5) {
    ...
    if (loglik > -Inf) {
      # in all cases we integrate from the origin of the island to the
      # colonization event (stac 2, 3, 4), the first branching point
      # (stac = 6, 7) or to the present (stac = 0, 1, 5)
      probs <- rep(0, 2 * lx + 1)
      probs[1] <- 1
      k1 <- 0
      y <- DAISIE_integrate(
      ...
      probs <- cp[[2]]
      if (stac == 0) {
        # for stac = 0, the integration is from the origin of the island
        # until the present
        # and we evaluate the probability of no clade being present and no
        # immigrant species,
        ...
        if (stac == 1 || stac == 5) {
          ...
          # and we evaluate the probability of the immigrant
          # species being present, but there can be missing species
          # for stac = 5, we do exactly the same,
          # but we evaluate the probability of an endemic species
          # being present alone.
          probs[(lx + 1):(2 * lx)] <- 0
          y <- DAISIE_integrate(
            probs,
          ...
          loglik <- cp[[1]]
          probs <- cp[[2]]
          loglik <- loglik + log(probs[(stac == 1) *
                                         lx + (stac == 5) + 1 + missnumspec])
        } else {
          # for stac = 2, 3, 4, integration is then from the colonization event
          # until the first branching time (stac = 2 and 3) or
          # the present (stac = 4). We add a set of equations for Q_M,n,
          # the probability that the process is compatible with the data,
          # and speciation has not happened; during this time immigration
          # is not allowed because it would alter the colonization time.
          ...
          # speciation to happen. We also multiply by the
          # (possibly diversity-dependent) immigration rate
          # for stac = 6 and 7, integration is from the maximum colonization
          # time until the first branching time
          if (stac == 6 || stac == 7) {
            probs[(lx + 1):(2 * lx)] <- 0
            y <- DAISIE_integrate(
              probs,
            ...
            probs <- y[2, 2:(2 * lx + 2)]
            cp <- checkprobs(lv = 2 * lx, loglik, probs, verbose)
            loglik <- cp[[1]]
            probs <- cp[[2]]
            k1 <- 1
          }
          if (stac == 2 || stac == 3 || stac == 4) {
            ...
            gamvec <- divdepvec(
              gam,
              c(pars1_in_divdepvec_call, t, 0),
              lx,
              k1,
              ddep * (ddep == 11 | ddep == 21),
              island_ontogeny
            ) # Problem may be here 30/3
          ...
          if (stac == 4) {
            ...
            # for stac = 2 and 3, at the first branching point all
            #  probabilities of states Q_M,n are transferred to probabilities
            #  where only endemics are present.
            #  Then go through the branching points.
            ...
            if (S1 >= startk) {
              ...
              if (stac == 6 || stac == 7) {
                probs2 <- rep(0, 2 * lx + 1)
                probs2[(1:(lx - 1))] <- probs[(2:lx)] + 1 / (2:lx) *
                  probs[(lx + 1):(2 * lx - 1)]
                probs2[lx] <- 1 / (lx + 1) * probs[2 * lx]
                probs2[(lx + 1):(2 * lx - 1)] <- (1:(lx - 1)) /
                  (2:lx) * probs[(lx + 2):(2 * lx)]
                probs <- probs2
                rm(probs2)
            ...
            # we evaluate the probability of the phylogeny with any missing
            # species at the present without (stac = 2 or stac = 6) or with
            # (stac = 3 or stac = 7) the immigrant species
            loglik <- loglik + log(probs[(stac == 3 || stac == 7) *
                                          lx + 1 + missnumspec])
          }
        }
      }
  ...
  if (pars2[4] >= 1) {
    if (length(pars1) == 11) {
      # CHANGE
      s1 <- sprintf(
        "Status of colonist: %d, Parameters: %f %f %f %f %f %f",
        stac,
        pars1[5],
        pars1[6],
        pars1[7],
...
DAISIE_loglik_CS_choice <- function(pars1,
                                   pars2,
                                   brts,
                                   stac,
                                   missnumspec,

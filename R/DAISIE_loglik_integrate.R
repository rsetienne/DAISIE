  ...
  if(sign(CS_version) == -1) {
    rho <- function(DAISIE_par, DAISIE_dist_pars) {
      return(stats::dlnorm(x = DAISIE_par,
                           meanlog = log(DAISIE_dist_pars[1]) - (DAISIE_dist_pars[2]^2)/2,
                           sdlog = sqrt(log(1 + DAISIE_dist_pars[2] / DAISIE_dist_pars[1]^2))))
    }
  } else {
    rho <- function(DAISIE_par, DAISIE_dist_pars) {
      return(stats::dgamma(x = DAISIE_par,
                           shape = DAISIE_dist_pars[2]^2,
                           scale = DAISIE_dist_pars[1]/DAISIE_dist_pars[2]^2))
    }
  }
  ...
  DAISIE_loglik_integrand <- function(DAISIE_par,
    ...
    loglik_DAISIE_par <- exp(DAISIE_loglik(
      ...
      rho(
        ...
        DAISIE_dist_pars = c(mean_par,k_or_sigma)
      )

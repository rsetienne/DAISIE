...
DAISIE_DE_loglik_CS <- function(
  ...
  cond = pars2[3]
  if (length(pars1) == 5) {
    logp0 = DAISIE_DE_logp0(datalist, pars1, methode, equal_extinction)
    ...
    logp0_type1 = DAISIE_DE_logp0(datalist, pars1[1:5], methode, equal_extinction)
    logp0_type2 = DAISIE_DE_logp0(datalist, pars1[6:10], methode, equal_extinction)
  ...
  for (i in 2:length(datalist)) {
    likelihood <- switch(datalist[[i]]$stac,
                         `1` = DAISIE_DE_logpNE_max_age_coltime(datalist, i, pars1, methode, rtol, atol, equal_extinction),
                         `2` = if (length(datalist[[i]]$branching_times) == 2 && datalist[[i]]$missing_species == 0) DAISIE_DE_logpES(datalist, i, pars1, methode, rtol, atol, equal_extinction) else DAISIE_DE_logpEC(datalist, i, pars1, methode, rtol, atol, equal_extinction),
                         `3` = if (length(datalist[[i]]$branching_times) == 2 && datalist[[i]]$missing_species == 0) DAISIE_DE_logpES_mainland(datalist, i, pars1, methode, rtol, atol, equal_extinction) else DAISIE_DE_logpEC_mainland(datalist, i, pars1, methode, rtol, atol, equal_extinction),
                         `4` = DAISIE_DE_logpNE(datalist, i, pars1, methode, rtol, atol, equal_extinction),
                         `5` = DAISIE_DE_logpES_max_age_coltime(datalist, i, pars1, methode, rtol, atol, equal_extinction),
                         `6` = if (length(datalist[[i]]$branching_times) > 2) DAISIE_DE_logpEC_max_age_coltime(datalist, i, pars1, methode, rtol, atol, equal_extinction) else function_Factor_Loglik_EC_max_Age_approximation_2(datalist, i, pars1)
    )

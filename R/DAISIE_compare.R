...
#'   \code{DAISIE_sim}
#' @param TRASIE_output4 List with simulation output using TRASIE pars
#'   \code{DAISIE_sim}
...
#' @param Tpars4 A named list containing diversification rates considering two trait states:
...
compare_diff <- function(
  ...
  for(i in 1:length(Data)){
    ...
    Data_differ_quantile <- apply(Data[[i]][,6:9],2,function(x) round(quantile(x,c(0.025,0.5,0.975)),digits=2))
    rownames(Data_differ_quantile)[1:3] <-c("differ_down","differ_median","differ_up")
    Data_t <- data.frame(t(Data_differ_quantile))

    pairs_name <- c("DAISIE vs. TRASIE_notrans","DAISIE vs. TRASIE_trans1","DAISIE vs. TRASIE_trans2","DAISIE vs. TRASIE_trans3")

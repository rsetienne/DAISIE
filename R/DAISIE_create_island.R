#' Converts oceanic simulation output into island output
#'
...
DAISIE_create_island_oceanic <- function(stt_table,
                                         totaltime,
                                         island_spec,
                                         mainland_n,
  ...
  if (length(island_spec[,1]) == 0) {
    ...
    if (mainland_n == 1) {
      island <- DAISIE_ONEcolonist_oceanic(totaltime,
                                           island_spec,
                                           stt_table,
      ...
      for (i in 1:number_colonists_present) {
        ...
        island_clades_info[[i]] <- DAISIE_ONEcolonist_oceanic(totaltime,
...
#' Converts nonoceanic simulation output into island output
#'
#' @param stt_table a species-through-time table
#' @param totaltime simulated amount of time
#' @param island_spec matrix with species on island (state of system at each time point)
...
DAISIE_create_island_nonoceanic <- function(stt_table,
                                            totaltime,
                                            island_spec,
                                            mainland_n,
                                            keep_final_state = FALSE,
  ...
  if (length(island_spec[,1]) == 0) {
    ...
    if (mainland_n == 1) {
      island <- DAISIE_ONEcolonist_nonoceanic(totaltime,
                                              island_spec,
                                              stt_table,
                                              keep_final_state = keep_final_state,
                                              nonend_spec,
      ...
      for (i in 1:number_colonists_present) {
        ...
        island_clades_info[[i]] <- DAISIE_ONEcolonist_nonoceanic(totaltime,
                                                                 island_spec = subset_island,
                                                                 stt_table = NULL,
                                                                 keep_final_state = keep_final_state,
                                                                 nonend_spec = nonend_spec,

...
DAISIE_format_CS_full_stt_SR <- function(island_replicates,
                                         time,
                                         M,
                                         verbose = TRUE
) {
  ...
  for (rep in seq_along(island_replicates)) {
    ...
    if (number_type2_cols > 0) {
      ...
      stt_type2 <- create_full_CS_stt_SR(
...
create_full_CS_stt_SR <- function(stt_list, stac_vec, totaltime) {
  ...
  # } else {
  # CONTINUE HERE
  extra_buffer_line <- rep(list(matrix(ncol = 5, c(
    totaltime + 1, 0, 0, 0, 0
  ))), length(filled_stt_lists))
  colnames(extra_buffer_line[[1]]) <- c("Time", "nI", "nA", "nC", "present")
  lapply(X = extra_buffer_line, FUN = rbind, filled_stt_lists)
  # do.call(what = rbind, args = list(extra_buffer_line, filled_stt_lists))
  ...
  full_stt <- as.data.frame(do.call(rbind, filled_stt_lists))

  ordered_diffs <- full_stt[order(full_stt$Time, decreasing = TRUE), ]
  # }
...
#'   filled_stt_lists <- stt_list[!zeros_second_line]
#'
#'   # Sums preexisting species on the island for SR case
#'   first_line_stts <- lapply(stt_list, "[", 1,)
#'   first_list_sums <- sapply(first_line_stts, sum)
#'   populated_from_start_stt <- stt_list[which(first_list_sums > totaltime)]
#'   populated_from_start_stt_start <- lapply(populated_from_start_stt, "[", 1,)
#'   populated_sum <- Reduce(`+`, populated_from_start_stt_start)
#'
#'   # Calculate 'present' and append to filled_stt_list
#'   num_indep_colonists <- list()
#'   for (i in seq_along(filled_stt_lists)) {
...
#'   } else {
#'
#'     filled_stt_lists_without_first <- lapply(filled_stt_lists, "[", -1,)
#'     # filled_stt_lists <- filled_stt_lists_without_first
#'     deltas_matrix <- lapply(filled_stt_lists, FUN = diff)
#'     for (i in seq_along(filled_stt_lists)) {
#'       deltas_matrix[[i]] <- rbind(
#'         filled_stt_lists[[i]][1, ],
...
#'   # Remove final duplicate lines, if any
#'   while (
#'     all(complete_stt_table[nrow(complete_stt_table) - 1, ] ==
#'         complete_stt_table[nrow(complete_stt_table), ])
#'   ) {
#'     complete_stt_table <- stt[1:(nrow(stt) - 1), ]
#'   }
#'   return(stt)
#' }
